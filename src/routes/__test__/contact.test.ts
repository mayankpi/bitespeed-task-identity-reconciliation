...
it("updates linkedId to primary if secondary records referring to each other instead of primary record", async () => {
    ...
    //find linkedId of sec2 and sec3
    let result = await Contact.findByPk(sec2.id);
    expect(result?.linkedId).toEqual(primary.id);
